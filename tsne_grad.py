# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 18:54:08 2016
...
def tsne_grad(x, X, P, network, v):
    ...
    for i in range(no_layers):
        ...
        ii += network[i]["W"].size
        network[i]["bias_upW"] = x[ii:ii+network[i]["bias_upW"].size].reshape(network[i]["bias_upW"].shape,order='F')
    ...
    activations.append(np.dot(activations[no_layers-1],np.vstack((network[no_layers-1]["W"],network[no_layers-1]["bias_upW"])))) 
